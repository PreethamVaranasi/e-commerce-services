...
def add(request,id,token):
    if not validate_user_session(id,token):
        return JsonResponse({'error':'Please Login to continue','code':'500'})
    if request.method == 'POST':
        user_id = id
        transaction_id = reuest.POST['transaction_id']
        amount = reuest.POST['amount']
        products = reuest.POST['products']
